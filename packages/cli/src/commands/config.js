...
const handlerAsync = async (environment, { params: configFilename, term: fromTermId }) => {
  ...
  const receipt = await court.instance.setConfig(
    ...
    [
      config.court.guardianFee.toString(),                    // guardianFee Amount of fee tokens that is paid per guardian per dispute
      config.court.draftFee.toString(),                       // draftFee Amount of fee tokens per guardian to cover the drafting cost
      config.court.settleFee.toString()                       // settleFee Amount of fee tokens per guardian to cover round settlement cost
    ],
    [
    ...
    [
      config.court.penaltyPct.toString(),                     // penaltyPct Permyriad of min active tokens balance to be locked for each drafted guardian (‱ - 1/10,000)
    ...
    [
      config.court.firstRoundGuardiansNumber.toString(),      // firstRoundGuardiansNumber Number of guardians to be drafted for the first round of disputes
      config.court.appealStepFactor.toString(),               // appealStepFactor Increasing factor for the number of guardians of each round of a dispute
      ...
      config.court.finalRoundLockTerms.toString()             // finalRoundLockTerms Number of terms that a coherent guardian in a final round is disallowed to withdraw (to prevent 51% attacks)
    ],
    [
